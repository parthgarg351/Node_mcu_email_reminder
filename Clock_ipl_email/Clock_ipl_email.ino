...
void displayFormattedText(String text) {
    ...
    for (int i = 0; i < text.length(); i++) {
        if (text[i] == ' ' || i == text.length() - 1) {
            if (i == text.length() - 1) word += text[i]; // Add last character if it's the last word
...
void displayNews(String news) 
{
    ...
    for (int i = 0; i < wordCount; i++) {
        if (pos + words[i].length() <= 20) {
            ...
            line++;
            if (line > 3) break; // Stop filling if the fourth line is reached
            currentLine = words[i] + " ";
            pos = words[i].length() + 1;
        }
        right=i;
    ...
    {   
        ...
        for (int i = right+1; i < wordCount; i++) 
        {
...
void fetchNews()
{
  ...
  HTTPClient http;
  http.setTimeout(60000);  // Set timeout to 60 seconds (60000 ms)
  Serial.println("Fetching News Data...");
